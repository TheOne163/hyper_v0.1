...
def parse_args(args, parser):
    ...
    # 检查参数是否已经存在
    if not any(action.dest == 'use_smooth_regularizer' for action in parser._actions):
        parser.add_argument("--use_smooth_regularizer", action='store_true', default=True,
                          help="Whether to use smooth regularizer")
    if not any(action.dest == 'use_align_regularizer' for action in parser._actions):
        parser.add_argument("--use_align_regularizer", action='store_true', default=True,
                          help="Whether to use align regularizer")
    if not any(action.dest == 'smooth_weight' for action in parser._actions):
        parser.add_argument("--smooth_weight", type=float, default=0.1,
                          help="Weight for smooth regularizer")
    if not any(action.dest == 'align_weight' for action in parser._actions):
        parser.add_argument("--align_weight", type=float, default=0.05,
                          help="Weight for align regularizer")

    all_args = parser.parse_known_args(args)[0]

    print(f"[Config] use_smooth_regularizer = {all_args.use_smooth_regularizer}")
    print(f"[Config] use_align_regularizer = {all_args.use_align_regularizer}")
    print(f"[Config] smooth_weight = {all_args.smooth_weight}")
    print(f"[Config] align_weight = {all_args.align_weight}")
