...
class SMACRunner(Runner):
    ...
    def eval(self, total_num_steps):
        eval_battles_won = 0
        eval_episode = 0
        eval_episode_rewards = []  # 存储每个回合的总奖励
        one_episode_rewards = [0.0] * self.n_eval_rollout_threads  # 每个线程的累积奖励
        ...
        while True:
            ...
            # 累加奖励
            for eval_i in range(self.n_eval_rollout_threads):
                one_episode_rewards[eval_i] += np.sum(eval_rewards[eval_i])  # 每个线程的奖励总和

            eval_dones_env = np.all(eval_dones, axis=1)

            # 重置结束环境的 rnn_states 和 masks
            ...
            for eval_i in range(self.n_eval_rollout_threads):
                if eval_dones_env[eval_i]:
                    eval_episode += 1
                    eval_episode_rewards.append(one_episode_rewards[eval_i])  # 记录该回合的总奖励
                    one_episode_rewards[eval_i] = 0.0  # 重置
                    if eval_infos[eval_i][0].get('won', False):  # 使用 .get() 避免 KeyError
                        eval_battles_won += 1

            # 达到评估回合数后退出
            if eval_episode >= self.all_args.eval_episodes:
                eval_avg_reward = np.mean(eval_episode_rewards)  # 计算平均回合奖励
                eval_win_rate = eval_battles_won / eval_episode if eval_episode > 0 else 0.0

                # 打印评估指标并强制刷新输出
                print(f"Eval Win Rate: {eval_win_rate}, Avg Episode Reward: {eval_avg_reward}")
                sys.stdout.flush()

                # 记录到 wandb 或 tensorboard
                if self.use_wandb:
                    wandb.log({"eval_win_rate": eval_win_rate, "eval_average_episode_reward": eval_avg_reward}, step=total_num_steps)
                else:
                    self.writter.add_scalars("eval_win_rate", {"eval_win_rate": eval_win_rate}, total_num_steps)
                    self.writter.add_scalars("eval_average_episode_reward", {"eval_average_episode_reward": eval_avg_reward}, total_num_steps)
